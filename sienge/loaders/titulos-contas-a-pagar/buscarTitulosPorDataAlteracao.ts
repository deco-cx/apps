...
export interface Props {
  /**
   * @title Data Inicial de Alteração
   * @description Data inicial de alteração para consulta de títulos (formato yyyy-MM-dd)
   */
  startDate: string;

  /**
   * @title Data Final de Alteração
   * @description Data final de alteração para consulta de títulos (formato yyyy-MM-dd)
   */
  endDate: string;
...
/**
 * @title Buscar Títulos por Data de Alteração
 * @description Retorna uma lista de títulos do contas a pagar que foram alterados no período especificado
 */
const loader = async (
  props: Props,
  _req: Request,
  ...
  const response = await billDebtClient["GET /bills/by-change-date"]({
