...
import { ToolCall, ToolResult } from "npm:@ai-sdk/provider-utils";

export type JSONValue = string | number | boolean | null | JSONValue[] | {
  [key: string]: JSONValue;
};

export type DataStreamString = `${(typeof DataStreamStringPrefixes)[
  keyof typeof DataStreamStringPrefixes
]}:${string}\n`;
...
> = {
  ...
  parse: (value: JSONValue) => {
    ...
    if (
      ...
      result.usage = {
        promptTokens: typeof value.usage.promptTokens === "number"
          ? value.usage.promptTokens
          : Number.NaN,
        completionTokens: typeof value.usage.completionTokens === "number"
...
export const dataStreamPartsByCode = Object.fromEntries(
  ...
  [K in (typeof dataStreamParts)[number]["code"]]:
    (typeof dataStreamParts)[number];
};
