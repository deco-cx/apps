...
export function createErrorHandler(options: ErrorHandlerOptions): ErrorHandler {
  ...
  const extractCleanErrorMessage = (message: string): { message: string, status?: number } => {
    try {
      ...
      if (parsed.error && typeof parsed.error === 'object') {
        ...
        } else if (parsed.error.status && !errorMessage.includes(parsed.error.status)) {
  ...
  const processResponseError = async (
    response: Response, 
