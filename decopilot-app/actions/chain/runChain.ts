...
import type { Attachment, LLMChainResponseType, LLMResponseType, Provider, TextOnly } from "../../types.ts";
...
export default async function action(
  ...
  if (chain.chainType === "Simple") {
    ...
    for (const block of chain.blockNames) {
      if (block.blockType === "Prompt") {
        ...
        const selected_prompt = getSavedPrompts({ name: block.blockNames }, _req, ctx)

        if (!selected_prompt) {
          throw new Error(`Prompt with Agent Name ${block.blockNames} not found`);
        }
        ...
        const response = await runPrompt({ called_prompt: selected_prompt.name, attachments}, _req, ctx);
        ...
        const response_message = response.llm_response.map(
              resp => resp?.message?.content).filter(
                  content => content !== null
              ) as string[];
        
              runAttachments = [reassembleAttachmentToText(response_message.join("\n"))]
              console.log(runAttachments)
  ...
  if (!runResponse) {
    throw new Error("No valid response was received during the chain execution.");
  }
