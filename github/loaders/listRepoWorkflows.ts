...
const loader = async (
  ...
  const result = await response.json() as { workflows: Record<string, unknown>[]; total_count: number };
  ...
  return {
    ...
    metadata: {
      ...
      has_next_page: props.per_page ? result.workflows.length === props.per_page : undefined,
    },
  };
};
