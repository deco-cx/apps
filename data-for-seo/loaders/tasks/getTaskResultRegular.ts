...
export interface RegularTaskResult {
  keyword: string;
  type: string;
  se_domain: string;
...
/**
 * @title Get Task Result - Regular
 * @description Retrieves regular SERP results for a specific task ID
 */
export default async function loader(
  ...
  try {
    ...
    return task.result[0] as RegularTaskResult;
  } catch (error) {
    throw new Error(
      `Failed to get regular task result: ${
