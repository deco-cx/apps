...
export default async function action(
  ...
  for (const keyword of categoryKeywords) {
    ...
    try {
      ...
      // Get Organic results
      const organicResponse = await ctx.api["POST /serp/google/organic/task_post"](
        {},
        {
          body: [{
            keyword: searchQuery,
      ...
      if (mapsTaskId) {
        const mapsResultResp = await ctx.api[`GET /serp/google/organic/task_get/:id`]({
          "id": mapsTaskId
        });
        
        const mapsResultData = await mapsResultResp.json();
        const mapsResult = mapsResultData.tasks?.[0]?.result?.[0] as { items?: MapsResult[] } | undefined;
      ...
      if (organicTaskId) {
        const organicResultResp = await ctx.api[`GET /serp/google/organic/task_get/:id`]({
          "id": organicTaskId
        });
        
        const organicResultData = await organicResultResp.json();
        const organicResult = organicResultData.tasks?.[0]?.result?.[0] as { items?: SerpItem[] } | undefined;
  ...
  const visibilityScore = calculateLocalVisibilityScore(localRankings, localPackAppearances);

  // Process competitor data
  const competitorAnalysis = Array.from(competitorData.entries()).map(([name, data]) => ({
    ...
    review_count: data.reviews.length > 0 
      ? Math.round(data.reviews.reduce((a, b) => a + b, 0) / data.reviews.length)
      : 0,
...
function identifyLocalOpportunities(
  ...
  const missingKeywords = rankings
    ...
    .map(r => ({
      ...
      competition: r.competitors_in_pack.length > 2 ? "high" as const : 
                   r.competitors_in_pack.length > 0 ? "medium" as const : "low" as const,
      current_leaders: r.competitors_in_pack,
    }));

  // Calculate review gap
  const competitorAvgReviews = competitors.length > 0
    ? competitors.reduce((sum, c) => sum + c.review_count, 0) / competitors.length
    : 100;
  ...
  const reviewGap = {
    ...
    reviews_needed: Math.max(0, Math.round(competitorAvgReviews * 1.2 - yourReviews)),
  };
...
function generateLocalRecommendations(
  ...
  if (opportunities.review_gap.reviews_needed > 0) {
    immediate.push(`Generate ${opportunities.review_gap.reviews_needed} new reviews to match competitor average`);
  }

  // Local pack optimization
  const notInPack = rankings.filter(r => r.has_local_pack && !r.maps_position).length;
  if (notInPack > 0) {
    immediate.push(`Optimize Google Business Profile for ${notInPack} keywords with local pack`);
  }

  // Optimization tips
  optimizationTips.push("Ensure NAP (Name, Address, Phone) consistency across all directories");
  optimizationTips.push("Add high-quality photos to Google Business Profile weekly");
  optimizationTips.push("Respond to all reviews within 24-48 hours");
  
  if (opportunities.missing_keywords.length > 0) {
    optimizationTips.push("Update business description with missing category keywords");
  }
  ...
  contentIdeas.push("Create area-specific service pages with local schema markup");

  // Top competitors to monitor
  const topCompetitor = competitors.sort((a, b) => b.appearances - a.appearances)[0];
  if (topCompetitor) {
    immediate.push(`Monitor and analyze ${topCompetitor.name}'s local SEO strategy`);
  }
