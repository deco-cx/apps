...
export const isFreshCtx = <TState>(
	ctx: ConnInfo | Context<TState>,
): ctx is Context<TState> => {
	return typeof (ctx as Context<TState>).render === "function";
};
...
export default function Fresh(
	...
	return async (req: Request, ctx: ConnInfo) => {
		 ...
		 */
		const firstByteThreshold =
			!asJson && delay && !appContext.isBot
				? delay === 1
					? (() => {
							console.warn(
		...
		try {
			const getPage = RequestContext.bind({ signal: ctrl.signal }, async () =>
				isDeferred<
					Page,
					BaseContext & {
						context: ConnInfo;
								...
								hooks: {
									onPropsResolveStart: (resolve, _props, resolver) => {
										...
										if (resolver?.type === "matchers") {
											// matchers should not have a timeout.
			...
			if (isFreshCtx<DecoState>(ctx)) {
				const timing =
					appContext?.monitoring?.timings?.start?.("render-to-string");
					...
					async (span) => {
						try {
							...
							const response = await renderToString({
								page:
									appContext.flavor && page
										? errorIfFrameworkMismatch(
												appContext.flavor?.framework,
												page,
											)
										: page,
								routerInfo: {
			...
			return Response.json(
				{ message: "Fresh is not being used" },
				{
					status: 500,
				},
			);
