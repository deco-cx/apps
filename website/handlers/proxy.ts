      ...
      async transform(chunk, controller) {
        ...
        if (cur != "") {
          for (let i = 0; i < chunk.length; i++) {
            cur = cur.slice(1, 6) + decoder.decode(chunk.slice(i, i + 1));

            if (cur == "<head>") {
              cur = "";

              cur += decoder.decode(chunk.slice(0, i + 1));
              cur += link1;
              cur += link2;
              cur += plausibleScript;
              cur += decoder.decode(chunk.slice(i + 1, chunk.length));

              newChunk = encoder.encode(cur);
              cur = "";
              break;
            }
          }
