...
export default async function BlogPostPageLoader(
  ...
  if (post.spirePostId && ctx.allowedBlogSlug && slug) {
    const spireBase = (ctx.spireUrl ?? "https://spire.blog")
      .replace(/\/+$/, "")
      .replace(/\/api$/, "");
    const fresh = await fetchSpireContent(ctx.allowedBlogSlug, slug, spireBase)
      .catch(() => null);
    if (fresh !== null) content = fresh; // use live API content
    // else: keep block-stored content (toLatinSafe fallback)
  }

  const fullPost: BlogPost = content !== post.content ? { ...post, content } : post;
