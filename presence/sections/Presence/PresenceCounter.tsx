...
export default function PresenceCounter(props: Props) {
  ...
  useEffect(() => {
    const ws = new WebSocket(`ws://${location.host}/presence?roomId=${location.pathname}`);
  ...
  return (
    <span
      ...
      class={`
        ${props.variant === "Day" ? "bg-base-100 text-primary" : 
          props.variant === "Dark" ? "bg-primary text-base-100" : 
