...
export function detectImageType(buffer: Uint8Array): ImageType | null {
  ...
  // WebP signature
  if (buffer[0] === 0x52 && buffer[1] === 0x49 && buffer[2] === 0x46 && buffer[3] === 0x57) {
    return "image/webp";
  }
...
export function calculateDimensions(width: number, height: number, maxPixels = MAX_PIXELS) {
...
export async function compressImage(imageBuffer: Uint8Array): Promise<Uint8Array> {
  ...
  const imageData = await decode(imageBuffer.buffer);
  ...
  if (totalPixels > MAX_PIXELS) {
    ...
    const { width: newWidth, height: newHeight } = calculateDimensions(imageData.width, imageData.height);
    console.log("Resizing to:", { newWidth, newHeight, newPixels: newWidth * newHeight });
    
    const resizedData = await transform(
      { data: imageBuffer.buffer, mediaType: imageType },
      {
        width: newWidth,
        height: newHeight,
        fit: "contain",
  ...
  while (compressedBuffer.byteLength > MAX_SIZE && quality >= 20) {
    ...
    const currentImageData = await decode(compressedBuffer.buffer);
    const result = await transform(
      { data: compressedBuffer.buffer, mediaType: imageType },
      {
  ...
  if (compressedBuffer.byteLength > MAX_SIZE) {
    throw new Error(`Unable to compress image below size limit. Final size: ${compressedBuffer.byteLength} bytes`);
  }
...
export type ProcessImageOptions = {
  ...
  processFn: (buffer: Uint8Array) => Promise<any>;
  maxRetries?: number;
};
...
export async function processImageWithCompression({
  ...
  processFn,
  maxRetries = 2,
}: ProcessImageOptions): Promise<any> {
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      if (error instanceof Error && 
        ...
        console.log(`Compression attempt ${retryCount + 1}/${maxRetries} needed`);
        
        if (retryCount === maxRetries) {
          throw new Error(`Failed to process image after ${maxRetries} compression attempts: ${error.message}`);
        }
