...
export default async function loader(
  ...
  if (!accessToken) {
    return {
      error: true,
      message: "Autenticação necessária para listar transmissões ao vivo. O token de acesso não foi encontrado."
    };
  }
  ...
  try {
    ...
    url.searchParams.append("part", includeVideoDetails 
    ...
    if (broadcastId) {
      ...
      if (mine || channelId || broadcastStatus !== "all") {
        console.warn("Aviso: Ao buscar por broadcastId específico, outros filtros serão ignorados (mine, channelId, broadcastStatus)");
      }
    } else if (channelId) {
      ...
      if (broadcastStatus === "all") {
        return {
          error: true,
          message: "Ao usar channelId, você deve especificar um broadcastStatus diferente de 'all'"
        };
      }
      ...
      if (mine) {
        console.warn("Aviso: O parâmetro 'mine' será ignorado porque 'channelId' foi especificado");
      }
    } else {
      ...
      if (broadcastStatus !== "all") {
        console.warn("Aviso: A API do YouTube não permite usar 'broadcastStatus' junto com 'mine=true'. O parâmetro 'broadcastStatus' será ignorado.");
      }
    }
    ...
    if (!response.ok) {
      ...
      console.error(`Erro HTTP ${response.status} ao listar transmissões:`, errorDetails);
      
      // Mensagens personalizadas para códigos de erro comuns
      let mensagem = `Erro ao listar transmissões: ${response.status} ${response.statusText}`;
      
      if (response.status === 401) {
        mensagem = "Token de autenticação inválido ou expirado. Faça login novamente.";
      } else if (response.status === 403) {
        mensagem = "Acesso negado. Verifique se você tem permissão para listar transmissões e o escopo 'youtube.livestream' está autorizado.";
      } else if (response.status === 404) {
        mensagem = "Recurso não encontrado. Verifique se os IDs das transmissões estão corretos.";
      } else if (response.status === 400) {
        mensagem = "Solicitação inválida. Verifique os parâmetros enviados.";
      } else if (response.status === 500) {
        mensagem = "Erro interno do servidor YouTube. Tente novamente mais tarde.";
    ...
    if (data.items.length === 0 && broadcastStatus !== "all") {
      return {
        ...
        infoMessage: `Nenhuma transmissão com status '${broadcastStatus}' encontrada. Tente outro status como 'upcoming' (agendadas), 'active' (ao vivo) ou 'completed' (finalizadas).`
      };
    }
    ...
    return {
      error: true,
      message: `Erro inesperado ao listar transmissões: ${error.message || "Erro desconhecido"}`,
      details: error
    };
  }
} 