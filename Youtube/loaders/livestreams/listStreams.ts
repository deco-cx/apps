...
export default async function loader(
  ...
  if (!accessToken) {
    return {
      error: true,
      message: "Autenticação necessária para listar streams. O token de acesso não foi encontrado."
    };
  }
  ...
  try {
    ...
    if (streamId) {
      ...
      return {
        error: true,
        message: "Parâmetros inválidos: pelo menos um dos parâmetros streamId ou mine deve ser especificado"
      };
    }
    ...
    if (!response.ok) {
      ...
      console.error(`Erro HTTP ${response.status} ao listar streams:`, errorDetails);
      
      // Mensagens personalizadas para códigos de erro comuns
      let mensagem = `Erro ao listar streams: ${response.status} ${response.statusText}`;
      
      if (response.status === 401) {
        mensagem = "Token de autenticação inválido ou expirado. Faça login novamente.";
      } else if (response.status === 403) {
        mensagem = "Acesso negado. Verifique se você tem permissão para listar streams e o escopo 'youtube.livestream' está autorizado.";
      } else if (response.status === 404) {
        mensagem = "Recurso não encontrado. Verifique se os IDs dos streams estão corretos.";
    ...
    if (data.items.length === 0) {
      return {
        ...
        infoMessage: "Nenhum stream de vídeo encontrado. Para transmissões ao vivo, você precisa primeiro configurar um stream de vídeo no canal."
      };
    }
    ...
    return {
      error: true,
      message: `Erro inesperado ao listar streams: ${error.message || "Erro desconhecido"}`,
      details: error
    };
  }
} 