...
export default async function action(
  ...
  try {
    ...
    // Criar o stream
    const url = "https://youtube.googleapis.com/youtube/v3/liveStreams?part=id,snippet,cdn,contentDetails,status";
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        message: `Erro ao criar stream: ${response.status} ${response.statusText}`,
        error: errorData
      };
    }
