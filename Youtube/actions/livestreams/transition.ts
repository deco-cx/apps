...
export default async function action(
  ...
  if (!["testing", "live", "complete"].includes(transitionType)) {
    return {
      success: false,
      message: "Tipo de transição inválido. Use 'testing', 'live' ou 'complete'"
    };
  }
  
  try {
    // Construir a URL para a transição
    const url = new URL("https://youtube.googleapis.com/youtube/v3/liveBroadcasts/transition");
    ...
    if (!response.ok) {
      ...
      let errorMessage = `Erro ao alterar status para ${transitionType}: ${response.status} ${response.statusText}`;
      
      if (transitionType === "testing" && response.status === 400) {
        errorMessage = "Erro ao iniciar teste: verifique se a transmissão está no estado 'ready' e se foi vinculada a um stream.";
      } else if (transitionType === "live" && response.status === 400) {
        errorMessage = "Erro ao ir ao vivo: verifique se a transmissão está no estado 'testing' e se o streaming está ativo.";
      } else if (transitionType === "complete" && response.status === 400) {
        errorMessage = "Erro ao finalizar: verifique se a transmissão está no estado 'live' ou 'testing'.";
      }
    ...
    return {
      success: false,
      message: `Erro ao alterar status da transmissão: ${error.message || "Erro desconhecido"}`,
      error
    };
  }
} 