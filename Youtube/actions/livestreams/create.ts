...
export default async function action(
  ...
  try {
    ...
    // Criar a transmissão
    const url = "https://youtube.googleapis.com/youtube/v3/liveBroadcasts?part=id,snippet,contentDetails,status";
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        message: `Erro ao criar transmissão: ${response.status} ${response.statusText}`,
        error: errorData
      };
    }
    ...
    return {
      success: false,
      message: `Erro ao criar transmissão: ${error.message || "Erro desconhecido"}`,
      error
    };
  }
} 