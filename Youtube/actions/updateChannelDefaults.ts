...
export default async function action(
  ...
  try {
    ...
    if (!updateResponse.ok) {
      ...
      return {
        success: false,
        message: `Erro ao atualizar padrões: ${updateResponse.status} ${updateResponse.statusText}`,
      };
    }
    ...
    console.error("Erro ao processar a atualização dos padrões do canal:", error);
    return {
      success: false,
