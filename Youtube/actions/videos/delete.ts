...
export default async function action(
  ...
  try {
    ...
    let mensagem = `Erro ao excluir vídeo: ${response.status} ${response.statusText}`;
    
    if (response.status === 401) {
      mensagem = "Token de autenticação inválido ou expirado. Faça login novamente.";
    } else if (response.status === 403) {
      mensagem = "Acesso negado. Verifique se você tem permissão para excluir este vídeo.";
    } else if (response.status === 404) {
      mensagem = "Vídeo não encontrado. O ID do vídeo pode estar incorreto ou o vídeo já foi excluído.";
