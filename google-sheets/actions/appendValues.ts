...
const action = async (
    ...
    try {
        ...
        if (!ctx.accessToken) {
            throw new Error("Token de acesso OAuth necessário para adicionar valores");
        }
        ...
        if (!response.ok) {
            ...
            throw new Error(`Erro ao adicionar valores: ${response.status} ${response.statusText} - ${errorText}`);
        }
