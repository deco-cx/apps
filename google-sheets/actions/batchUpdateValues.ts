...
import { BatchUpdateValuesRequest, BatchUpdateValuesResponse } from "../utils/types.ts";
...
const action = async (
    ...
    try {
        ...
        const body: BatchUpdateValuesRequest = {
            ...
            responseValueRenderOption: includeValuesInResponse ? responseValueRenderOption : undefined,
            responseDateTimeRenderOption: includeValuesInResponse ? responseDateTimeRenderOption : undefined,
            data: data.map((item) => ({
                range: item.range,
        ...
        if (!ctx.accessToken) {
            throw new Error("Token de acesso OAuth necessário para atualizar valores em lote");
        }
        ...
        if (!response.ok) {
            ...
            throw new Error(`Erro ao atualizar valores em lote: ${response.status} ${response.statusText} - ${errorText}`);
        }
