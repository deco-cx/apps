...
const loader = async (
    ...
    try {
        ...
        if (ctx.apiKey && !ctx.accessToken) {
            ...
            if (!response.ok) {
                throw new Error(`Erro ao obter valores: ${response.status} ${response.statusText}`);
            }
        ...
        const urlWithParams = new URL(`/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(range)}`, "https://sheets.googleapis.com");
        ...
        urlWithParams.searchParams.append("dateTimeRenderOption", dateTimeRenderOption);
