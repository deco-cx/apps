...
export interface Props extends Omit<UpdateEventParams, 'calendarId' | 'eventId'> {
  /** ID do calendário */
  calendarId: string;
  /** ID do evento a ser atualizado */
  eventId: string;
  /** Dados atualizados do evento */
  event: Partial<Omit<Event, 'id' | 'etag' | 'created' | 'updated' | 'htmlLink' | 'iCalUID' | 'sequence' | 'kind'>>;
...
export default async function updateEvent(
  ...
  const { calendarId, eventId, event, fullUpdate = false, ...searchParams } = props;
