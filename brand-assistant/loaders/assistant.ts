...
const BASE_INSTRUCTIONS =
  ...
  - If you found something in the the store that you think is relevant to the user's query, that means if you are answering without the need for asking for more information,  in your data that is near to what the user requested, start your response with an indication of success, like 'I found some products that might interest you!', followed by a brief description, and end your message with an ${Tokens.POSITIVE} symbol. Otherwise, add a ${Tokens.NEGATIVE} symbol. This symbols should appear the very end of the message, even after the last appeareance of ${Tokens.OPTIONS}.
  ...
  - Keep in mind the user's original query and preferences when presenting the products, ensuring that the information provided is both relevant and useful to the user's specific needs.
