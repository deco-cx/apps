...
export default function generateImageAction(
  ...
  if (props.quality) {
    if (
      props.quality !== "low" && props.quality !== "medium" &&
      props.quality !== "high" && props.quality !== "auto"
    ) {
      return {
        success: false,
        message:
          "Quality must be a valid quality value. Valid values are: low, medium, high, auto",
