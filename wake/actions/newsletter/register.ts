...
const action = async (props: Props, req: Request, ctx: AppContext): Promise<NewsletterNode> => {
    const { storefront } = ctx
    const headers = parseHeaders(req.headers)

    const data = await storefront.query<CreateNewsletterRegisterMutation, CreateNewsletterRegisterMutationVariables>(
        {
