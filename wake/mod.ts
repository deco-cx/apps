...
export default function App(props: Props): App<Manifest, State> {
    const { token, storefrontToken, account, checkoutUrl, useCustomCheckout = false } = props
    ...
    const stringStorefrontToken = typeof storefrontToken === 'string' ? storefrontToken : storefrontToken?.get?.() ?? ''
...
export const preview = previewFromMarkdown(new URL('./README.md', import.meta.url))
