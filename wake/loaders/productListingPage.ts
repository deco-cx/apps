 ...
 */
const searchLoader = async (props: Props, req: Request, ctx: AppContext): Promise<ProductListingPage | null> => {
    // get url from params
    const url =
    ...
    const sort =
        ...
        props.sort ??
        'SALES:DESC'
    const page = props.page ?? Number(url.searchParams.get('page')) ?? Number(url.searchParams.get('pagina')) ?? 0
    ...
    const hasNextPage = Boolean(
        (data?.result?.productsByOffset?.totalCount ?? 0) / (data?.result?.pageSize ?? limit) >
    ...
    const variations = props.getVariations ? await getVariations(storefront, productIDs, headers, url) : []
    ...
    return {
        ...
        products: products
            ...
            .map(variant => {
                const productVariations = variations?.filter(v => v.inProductGroupWithID === variant.productId)
