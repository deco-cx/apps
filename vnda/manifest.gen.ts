// DO NOT EDIT. This file is generated by deco.
// This file SHOULD be checked into source version control.
// This file is automatically updated during development when running `dev.ts`.

import * as $$$0 from "./loaders/productListingPage.ts";
import * as $$$1 from "./loaders/proxy.ts";
import * as $$$2 from "./loaders/cart.ts";
import * as $$$3 from "./loaders/productDetailsPage.ts";
import * as $$$4 from "./loaders/productList.ts";
import * as $$$$$$$$$0 from "./actions/cart/simulation.ts";
import * as $$$$$$$$$1 from "./actions/cart/updateItem.ts";
import * as $$$$$$$$$2 from "./actions/cart/addItem.ts";
import * as $$$$$$$$$3 from "./actions/cart/updateCart.ts";

const manifest = {
  "loaders": {
    "vnda/loaders/cart.ts": $$$2,
    "vnda/loaders/productDetailsPage.ts": $$$3,
    "vnda/loaders/productList.ts": $$$4,
    "vnda/loaders/productListingPage.ts": $$$0,
    "vnda/loaders/proxy.ts": $$$1,
  },
  "actions": {
    "vnda/actions/cart/addItem.ts": $$$$$$$$$2,
    "vnda/actions/cart/simulation.ts": $$$$$$$$$0,
    "vnda/actions/cart/updateCart.ts": $$$$$$$$$3,
    "vnda/actions/cart/updateItem.ts": $$$$$$$$$1,
  },
  "name": "vnda",
  "baseUrl": import.meta.url,
};

export type Manifest = typeof manifest;

export default manifest;
